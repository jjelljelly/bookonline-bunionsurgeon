        ...
        <div className={style.calendar}>
                    <div className={style.calUiContainer}>
                        ...
                        <div className={style.calHeader}>
                            <button className={style.calNavButton}
                        ...
                        <div className={style.dayNames}>
                            {
                                tDays.map((day, i) => {
                                    return <div key={"day-name" + i} className={style.dayName}>{day}</div>
                                })
                            }
                        </div>
                        <div className={style.calDays}>
                            {
                                                    ...
                                                    style={ (day === date.getDate() && currentMonth === date.getMonth()) ? //if the date is the selected date
                                                                {backgroundColor: 'var(--see-through)', color: 'var(--the-black)'} : 
                                                                    (new Date(currentYear, currentMonth, day) <= new Date() || !isAvailable) ? //if the date is before todays date
                                                                        {color: 'white', backgroundColor: 'var(--see-through)', cursor: 'default'} :
                                                                        {width: '14%'}}
                                                    >
                                                        {day}
                                                </button>
